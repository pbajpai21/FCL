...
AVAILABLE_ALGOS = ["fedavg", "fedfa", "fedbuff", "pf", "la", "lapf"]


def parse_args():
    parser = argparse.ArgumentParser(
        description="EMNIST FedAvg/FedFa variants comparison."
    )
    parser.add_argument(
        "--algorithms",
        nargs="+",
        default=AVAILABLE_ALGOS,
        help=f"Algorithms to run. Choices: {', '.join(AVAILABLE_ALGOS)}. Default: all.",
    )
    parser.add_argument("--num-clients", type=int, default=15)
    ...
    invalid = sorted(set(args.algorithms) - set(AVAILABLE_ALGOS))
    if invalid:
        raise ValueError(f"Unsupported algorithms: {invalid}. Supported: {AVAILABLE_ALGOS}")
    return args
...
event_indices_lapf: list = []
event_accuracies_lapf: list = []
event_losses_lapf: list = []
...
class FedFaServerLabelParticipationFair:
    """
    Simple combined strategy: Label-Aware + Participation-Fair.
      - Keeps latest model per client in buffer.
      - Weight for client i: w_i ~ label_score_i * (1 / sqrt(participation_i + eps))
      - Normalized weighted average when buffer is full.
    """
    ...
    def apply_fedfa_update(self, client_params, client_id: int):
        ...
        for e in self.buffer:
            cid = e["client_id"]
            w_label = max(self.client_label_scores.get(cid, 1.0), self.eps)
            w_part = 1.0 / math.sqrt(float(self.participation_counts[cid]) + self.eps)
            raw_weights.append(max(w_label * w_part, self.eps))
        Z = sum(raw_weights)
        norm_weights = [w / Z for w in raw_weights]
...
global_model_fedavg = None
if "fedavg" in selected_algorithms:
    print(f"\n================ FedAvg (synchronous) ================\n")
...
rounds_fedavg = num_events // num_clients if global_model_fedavg is not None else 0  # roughly same number of client updates as FedFa
...
# print(f"=" * 80)
if global_model_fedavg is not None:
    for rnd in range(1, rounds_fedavg + 1):
        # Local training on all clients from the current global model
        client_states = []
        client_sizes = []
        ...
        for client in clients:
            ...
            local_loader = DataLoader(local_dataset, batch_size=local_batch_size, shuffle=True)
...
if global_model_fedavg is not None:
    ...
    if fedavg_accuracies:
        ...
        print(f"   Avg Accuracy: {sum(fedavg_accuracies) / len(fedavg_accuracies):.4f} ({(sum(fedavg_accuracies) / len(fedavg_accuracies)) * 100:.2f}%)")
    else:
...
global_model_fedfa = None
server = None
if "fedfa" in selected_algorithms:
    print(f"\n================ FedFa (asynchronous) ================\n")
...
if global_model_fedfa is not None:
    print(f"\n🚀 Starting EMNIST FedFa-Style Fully Asynchronous Federated Learning (Non-IID)...")
    print(f"=" * 80)

for event in range(1, num_events + 1 if global_model_fedfa is not None else 0):
...
if global_model_fedfa is not None:
    ...
    if event_accuracies:
        ...
        print(f"   Avg Accuracy: {sum(event_accuracies) / len(event_accuracies):.4f} ({(sum(event_accuracies) / len(event_accuracies)) * 100:.2f}%)")
    else:
...
global_model_fedbuff = None
server_buff = None
if "fedbuff" in selected_algorithms:
...
for event in range(1, num_events + 1 if global_model_fedbuff is not None else 0):
    client = random.choice(clients)
...
if global_model_fedbuff is not None:
    print(f"\n" + "=" * 80)
...
global_model_fedfa_pf = None
server_pf = None
if "pf" in selected_algorithms:
    print(f"\n================ Participation-Fair FedFa (asynchronous) ================\n")
...
if global_model_fedfa_pf is not None:
    print(f"\n🚀 Starting EMNIST Participation-Fair FedFa Async Federated Learning (Non-IID)...")
    print(f"=" * 80)

for event in range(1, num_events + 1 if global_model_fedfa_pf is not None else 0):
    client = random.choice(clients)
...
global_model_fedfa_la = None
server_la = None
if "la" in selected_algorithms:
...
for event in range(1, num_events + 1 if global_model_fedfa_la is not None else 0):
    client = random.choice(clients)
...
if global_model_fedfa_la is not None:
    print(f"\n" + "=" * 80)
    ...
    if event_accuracies_la:
        ...
        print(f"   Avg Accuracy: {sum(event_accuracies_la) / len(event_accuracies_la):.4f} ({(sum(event_accuracies_la) / len(event_accuracies_la)) * 100:.2f}%)")
    else:
        print("\nNo Label-Aware FedFa evaluation points collected.")

# ========================================
# 🚀 Label+Participation Fair FedFa (ASYNC)
# ========================================

global_model_fedfa_lapf = None
server_lapf = None
if "lapf" in selected_algorithms:
    print(f"\n================ Label+Participation-Fair FedFa (asynchronous) ================\n")
    global_model_fedfa_lapf = MLPModel(num_classes=num_classes).to(DEVICE)
    print(f"\n📊 Global Model (EMNIST Label+Participation Fair FedFa):")
    print_model_stats(global_model_fedfa_lapf)
    server_lapf = FedFaServerLabelParticipationFair(
        global_model_fedfa_lapf,
        buffer_size=buffer_size,
        client_label_scores=client_label_scores,
    )
    for client in clients:
        client.pull_from_server(server_lapf.global_params, server_lapf.global_version)
    print(f"\n🚀 Starting EMNIST Label+Participation-Fair FedFa Async Federated Learning (Non-IID)...")
    print(f"=" * 80)

for event in range(1, num_events + 1 if global_model_fedfa_lapf is not None else 0):
    client = random.choice(clients)
    updated_params, used_version = client.train_async(
        base_model=global_model_fedfa_lapf,
        epochs=local_epochs,
        lr=local_lr,
        batch_size=local_batch_size,
    )
    staleness = server_lapf.global_version - used_version
    server_lapf.apply_fedfa_update(
        updated_params,
        client_id=client.client_id,
    )
    client.pull_from_server(server_lapf.global_params, server_lapf.global_version)
    ...
    if event % eval_every == 0 or event == 1:
        acc, loss = evaluate_global_model(global_model_fedfa_lapf, test_loader)
        event_indices_lapf.append(event)
        event_accuracies_lapf.append(acc)
        event_losses_lapf.append(loss)
        print(f"\n📊 After Label+Participation-Fair FedFa async event {event}/{num_events}:")
        print(f"   Used client: {client.client_id}, staleness: {staleness}")
        print(f"   Current Label+Participation-Fair FedFa buffer size: {len(server_lapf.buffer)}")
        print(f"   Global Accuracy (Label+Participation-Fair FedFa): {acc:.4f} ({acc * 100:.2f}%), Loss: {loss:.4f}")

if global_model_fedfa_lapf is not None:
    print(f"\n" + "=" * 80)
    print(f"EMNIST LABEL+PARTICIPATION-FAIR FEDFA FULLY ASYNC FEDERATED LEARNING RESULTS")
    print(f"=" * 80)
    if event_accuracies_lapf:
        print(f"\n📈 Final Performance (Label+Participation-Fair EMNIST FedFa):")
        print(f"   Final Accuracy: {event_accuracies_lapf[-1]:.4f} ({event_accuracies_lapf[-1] * 100:.2f}%)")
        print(f"   Best Accuracy: {max(event_accuracies_lapf):.4f} ({max(event_accuracies_lapf) * 100:.2f}%)")
        print(f"   Avg Accuracy: {sum(event_accuracies_lapf) / len(event_accuracies_lapf):.4f} ({(sum(event_accuracies_lapf) / len(event_accuracies_lapf)) * 100:.2f}%)")
    else:
        print("\nNo Label+Participation-Fair FedFa evaluation points collected.")

print("\n📊 Per-client accuracy on final global models (FedAvg vs FedFa / FedBuff / PF-FedFa / Label-Aware / LA+PF):")
...
for client in clients:
    ...
    if global_model_fedavg is not None:
        ...
        msg_parts.append(f"FedAvg={acc_fedavg:.4f} ({acc_fedavg * 100:.2f}%)")
    if global_model_fedfa is not None:
        acc_base = evaluate_model_on_client_data(global_model_fedfa, client)
        per_client_acc_baseline.append(acc_base)
        msg_parts.append(f"FedFa={acc_base:.4f} ({acc_base * 100:.2f}%)")
    if global_model_fedbuff is not None:
        acc_buff = evaluate_model_on_client_data(global_model_fedbuff, client)
        per_client_acc_buff.append(acc_buff)
        msg_parts.append(f"FedBuff={acc_buff:.4f} ({acc_buff * 100:.2f}%)")
    if global_model_fedfa_pf is not None:
        acc_pf = evaluate_model_on_client_data(global_model_fedfa_pf, client)
        per_client_acc_pf.append(acc_pf)
        msg_parts.append(f"PF-FedFa={acc_pf:.4f} ({acc_pf * 100:.2f}%)")
    if global_model_fedfa_la is not None:
        acc_la = evaluate_model_on_client_data(global_model_fedfa_la, client)
        per_client_acc_la.append(acc_la)
        msg_parts.append(f"LabelAware-FedFa={acc_la:.4f} ({acc_la * 100:.2f}%)")
    if global_model_fedfa_lapf is not None:
        acc_lapf = evaluate_model_on_client_data(global_model_fedfa_lapf, client)
        per_client_acc_lapf.append(acc_lapf)
        msg_parts.append(f"LA+PF-FedFa={acc_lapf:.4f} ({acc_lapf * 100:.2f}%)")
    print(f"   Client {client.client_id}: " + ", ".join(msg_parts))
...
if per_client_acc_lapf:
    mean_lapf = sum(per_client_acc_lapf) / len(per_client_acc_lapf)
    print(f"   LA+PF FedFa mean per-client accuracy: {mean_lapf:.4f} ({mean_lapf * 100:.2f}%)")
...
def plot_emnist_fedfa_results():
    if (not event_indices and
            ...
            not event_indices_la and
            not event_indices_lapf):
        return
    ...
    if event_indices_lapf:
        ax1.plot(event_indices_lapf, event_accuracies_lapf, 'c-s', linewidth=2, markersize=6,
                 label='LA+PF FedFa Accuracy')
    ...
    if event_indices_lapf:
        ax2.plot(event_indices_lapf, event_losses_lapf, 'c-s', linewidth=2, markersize=6,
                 label='LA+PF FedFa Loss')
...
print(
    f"\n📊 Creating EMNIST FedFa-Style Asynchronous Federated Learning Visualization (FedBuff vs FedFa vs PF-FedFa vs Label-Aware vs LA+PF)...")
...
_print_final_best("LA+PF FedFa (async)", event_accuracies_lapf)
...
if global_model_fedfa is not None and global_model_fedfa_la is not None:
