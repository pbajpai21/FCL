...
event_indices_va: list = []
event_accuracies_va: list = []
event_losses_va: list = []
...
print(f"EMNIST Balanced - num classes: {num_classes}")
...
def split_emnist_two_labels_per_client(dataset, num_clients: int):
    """
    Pathological non-IID:
      - Each client gets exactly 2 labels (disjoint when possible).
      - Clients see only those labels.
    """
    num_classes_local = len(dataset.classes)
    ...
    # Random permutation of all labels
    ...
    for cid in range(num_clients):
        start = 2 * cid
        labs = [
            all_labels[start % num_classes_local],
            all_labels[(start + 1) % num_classes_local],
        ]
...
print(f"   Non-IID pattern: pathological (2 labels per client)")

# Build pathological non-IID clients (2 labels per client), reuse for FedFa
clients, client_label_sets = split_emnist_two_labels_per_client(train_ds_full, num_clients)
...
# ========================================
# 🚀 Validation-Aware FedFa EXPERIMENT (ASYNC)
# ========================================

print(f"\n================ Validation-Aware FedFa (asynchronous) ================\n")

global_model_fedfa_va = MLPModel(num_classes=num_classes).to(DEVICE)
print(f"\n📊 Global Model (EMNIST Validation-Aware FedFa):")
print_model_stats(global_model_fedfa_va)

server_va = FedFaServerValAware(global_model_fedfa_va, buffer_size=buffer_size, val_loader=val_loader, alpha=5.0)

for client in clients:
    client.pull_from_server(server_va.global_params, server_va.global_version)

print(f"\n🚀 Starting EMNIST Validation-Aware FedFa Async Federated Learning (Non-IID)...")
print(f"=" * 80)
...
for event in range(1, num_events + 1):
    ...
    updated_params, used_version = client.train_async(
        base_model=global_model_fedfa_va,
        epochs=local_epochs,
        lr=local_lr,
        batch_size=32,
    )

    staleness = server_va.global_version - used_version

    server_va.apply_fedfa_update(
        updated_params,
        client_id=client.client_id,
    )
    ...
    if event % 20 == 0 or event == 1:
        acc, loss = evaluate_global_model(global_model_fedfa_va, test_loader)
        event_indices_va.append(event)
        event_accuracies_va.append(acc)
        event_losses_va.append(loss)
        print(f"\n📊 After Validation-Aware FedFa async event {event}/{num_events}:")
        print(f"   Used client: {client.client_id}, staleness: {staleness}")
        print(f"   Current Validation-Aware FedFa buffer size: {len(server_va.buffer)}")
        print(f"   Global Accuracy (Validation-Aware FedFa): {acc:.4f} ({acc*100:.2f}%), Loss: {loss:.4f}")


print(f"\n" + "=" * 80)
print(f"EMNIST VALIDATION-AWARE FEDFA FULLY ASYNC FEDERATED LEARNING RESULTS")
print(f"=" * 80)

if event_accuracies_va:
    print(f"\n📈 Final Performance (Validation-Aware EMNIST FedFa):")
    print(f"   Final Accuracy: {event_accuracies_va[-1]:.4f} ({event_accuracies_va[-1]*100:.2f}%)")
    print(f"   Best Accuracy: {max(event_accuracies_va):.4f} ({max(event_accuracies_va)*100:.2f}%)")
else:
    print("\nNo Validation-Aware FedFa evaluation points collected.")


print("\n📊 Per-client accuracy on final global models (baseline vs FedBuff vs Participation-Fair vs Label-Aware vs Validation-Aware):")
...
for client in clients:
    ...
    acc_va = evaluate_model_on_client_data(global_model_fedfa_va, client)
    ...
    per_client_acc_va.append(acc_va)
    print(f"   Client {client.client_id}: "
          ...
          f"ValAware-FedFa={acc_va:.4f} ({acc_va*100:.2f}%)")

if per_client_acc_buff:
    mean_buff = sum(per_client_acc_buff) / len(per_client_acc_buff)
    print(f"\n   FedBuff mean per-client accuracy: {mean_buff:.4f} ({mean_buff*100:.2f}%)")
...
if per_client_acc_va:
    mean_va = sum(per_client_acc_va) / len(per_client_acc_va)
    print(f"   Validation-Aware FedFa mean per-client accuracy: {mean_va:.4f} ({mean_va*100:.2f}%)")
...
def plot_emnist_fedfa_results():
    ...
    if event_indices_va:
        ax1.plot(event_indices_va, event_accuracies_va, 'm-s', linewidth=2, markersize=6, label='Val-Loss FedFa Accuracy')
    ...
    if event_indices_va:
        ax2.plot(event_indices_va, event_losses_va, 'm-s', linewidth=2, markersize=6, label='Val-Loss FedFa Loss')
...
print(f"\n📊 Creating EMNIST FedFa-Style Asynchronous Federated Learning Visualization (FedBuff vs FedFa vs Label-Aware vs Val-Loss)...")
